    ...
    setState(() {
      ...
      allCategory?.then((value) {
        selectedCategoryName =
            value.categories?[0].name?['${stctrl.code.value}'] ??
                value.categories?[0].name?['en'];
        selectedCategoryId = value.categories?[0].id;
      });
      allLevel = getLevels();
      allLevel?.then((levelValue) {
        selectedLevelName =
            levelValue.levels?[0].title?['${stctrl.code.value}'] ??
                levelValue.levels?[0].title?['en'];
            ...
            items: level.map((item) {
              return DropdownMenuItem<String>(
                ...
                child: Padding(
                  padding: const EdgeInsets.only(left: 8.0),
                  child: Text(
                      item.title?['${stctrl.code.value}'] ?? item.title?['en']),
                ),
              );
            }).toList(),
            onChanged: (value) {
